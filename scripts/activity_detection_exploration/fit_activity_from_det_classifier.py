...
pred_fnames = ['/angel_workspace/ros_bags/m2_all_data_cleaned_fixed_with_steps/m2_all_data_cleaned_fixed_with_steps_results_test.mscoco.json',
...
if True:
    fname = '/angel_workspace/model_files/recipe_m2_apply_tourniquet_v0.052.2.pkl'
    with open(fname, 'rb') as of:
        label_to_ind, 1, pipe, act_str_list = pickle.load(of)
if False:
    # Train model on test set.
    clf = RandomForestClassifier(max_depth=10, random_state=0, n_estimators=1000,
                                 max_features=0.1, bootstrap=True)
    clf.fit(X, y)
elif False:
    clf = LogisticRegression(random_state=0, C=0.0006951927961775606,
...
if False:
    ...
    tmp[0] = class_mean_conf;    tmp[1] = class_std_conf
...
for ds_id in sorted(list(set(dataset_id_final_test))):
    ...
    save_fname = '/angel_workspace/ros_bags/new/hmm-ds=%s.png' % dsets[ds_id].split('/')[-3]
    viz_step_finished_conf(live_model, y_pred_, y_, time_windows, valid,
