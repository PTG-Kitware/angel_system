...
class ObjectDetectorWithDescriptors_v2(Node):
    ...
    This version adds support for being able to perform inferencing with the
    newly trained models which were trained with a slightly different
    architecture and parameters than the pretrained model used previously.
    """
    ...
    def image_callback(self, image):
        ...
        if img_time_ns <= min_time:
            ...
            log.warn(
                f"Skipping frame with time {img_time_ns} ns <= min time "
                f"{min_time} ns"
            )
            return
    ...
    def postprocess_detections(
        ...
        box_deltas = (
            box_deltas.view(-1, 4) * torch.FloatTensor((0.1, 0.1, 0.2, 0.2)).cuda()
        ...
        for j in range(1, len(self.classes)):
            inds = (
                torch.nonzero(scores[:, j] > self._detection_threshold).view(-1).cpu()
            )
            ...
            if inds.numel() > 0:
                ...
                cls_boxes = torch.tensor(boxs_inds[:, j * 4 : (j + 1) * 4]).to(
