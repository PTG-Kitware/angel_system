...
class BaseEmotionDetector(Node):
    ...
    def __init__(self):
        ...
        self._in_interp_uintent_topic = self.get_parameter(
            IN_INTERP_USER_INTENT_TOPIC
        ).value
        self._in_expect_uintent_topic = self.get_parameter(
            IN_EXPECT_USER_INTENT_TOPIC
        ).value
        self._out_interp_uemotion_topic = self.get_parameter(
            OUT_INTERP_USER_EMOTION_TOPIC
        ).value
        self.log.info(
            f"Interpreted User Intents topic: "
            f"({type(self._in_interp_uintent_topic).__name__}) "
            f"{self._in_interp_uintent_topic}"
        )
        self.log.info(
            f"Expected User Intents topic: "
            f"({type(self._in_expect_uintent_topic).__name__}) "
            f"{self._in_expect_uintent_topic}"
        )
        self.log.info(
            f"Interpreted User Emotion topic: "
