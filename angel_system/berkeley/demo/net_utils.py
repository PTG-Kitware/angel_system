...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        if score > thresh:
            ...
            if class_name == 'hand':
                cv2.putText(im, '%s: %.3f lr %.1f s %.1f' % (class_name, score, lr, state), (bbox[0], bbox[1] + 15),
...
def vis_detections_filtered_objects(im, obj_dets, hand_dets, thresh=0.8):
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for i in range(np.minimum(10, hand_dets.shape[0])):
            ...
            if score > thresh:
                cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 204, 0), 2)
                cv2.putText(im, '%s: %.3f lr %.1f s %.1f' % ('hand', score, lr, state), (bbox[0], bbox[1] + 15),
                            cv2.FONT_HERSHEY_PLAIN,
                            1.0, (0, 0, 255), thickness=1)
                if state > 0:
            ...
            if score > thresh_hand:
                ...
                if state > 0:  # in contact hand
                    ...
                    if lr == 0:
                        ...
                        side_idx = 1
                    draw_line_point(draw, side_idx, (int(hand_cc[0]), int(hand_cc[1])),
...
def filter_object(obj_dets, hand_dets):
    ...
    for i in range(hand_dets.shape[0]):
        ...
        point_cc = np.array([(hand_cc[0] + hand_dets[i, 6] * 10000 * hand_dets[i, 7]),
