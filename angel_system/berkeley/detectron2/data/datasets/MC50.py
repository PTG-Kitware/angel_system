...
__all__ = ["load_MC50_json", "register_MC50_instances", "get_MC50_instances_meta"]


def register_MC50_instances(name, metadata, json_file, image_root):
    """
...
def load_MC50_json(json_file, image_root, dataset_name=None, meta=None, extra_annotation_keys=None):
    ...
    #     if not len(a) == 0:
    #         ann_new.append(a)
    #         imgs_new.append(imgs[jj])
    #     jj = jj + 1

    # Sanity check that each annotation has a unique id
    ann_ids = [ann["id"] for anns_per_image in anns for ann in anns_per_image]
    # ann_ids = []
    # for anns_per_image in anns:
    ...
    def get_file_name_ft(img_root, img_dict):
        # Determine the path including the split folder ("train2017", "val2017", "test2017") from
        # the coco_url field. Example:
        #   'coco_url': 'http://images.cocodataset.org/train2017/000000155379.jpg'
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        record = {}
        record["file_name"] = get_file_name_ft(image_root, img_dict)
...
def get_MC50_instances_meta(dataset_name):
    """
    Load LVIS metadata.
    ...
    if 'MC50' in dataset_name:
        return _get_MC50_instances_meta_v1()
...
if __name__ == "__main__":
    ...
    dicts = load_MC50_json(sys.argv[1], sys.argv[2], sys.argv[3])
