...
_BLACK = (0, 0, 0)
_RED = (1.0, 0, 0)
_CONNECTION_OBJ_OBJ = (255/255, 215/255, 0/255)
_CONNECTION_OBJ_HAND = (0/255, 255/255, 127/255)
_STEP_COLOR = (255/255, 0/255, 0/255)
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        # dets = np.array(boxes.tensor)
        # # dets[:, 2:4] += dets[:, 0:2]  # convert to [x1,y1,w,h] to [x1,y1,x2,y2]
        # # total_frames += 1
        # _class = classes
        # if not mot_tracker == None:
        ...
        self.overlay_instances_MC50(
            ...
            alpha=alpha,
            obj_obj_contact_scores = obj_obj_contact_scores,
    ...
    def draw_instance_predictions_smoothing(self, predictions, tracker, current_idx):
        """
        Draw instance-level prediction results on an image.
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            if _ASSIGN_COLOR:
                ...
                label_ = labels[i][: (-len_score - 1)]
                # color_mapping = self.load_color_mapping(label_)
                color = self.color_mapping[label_]

            if boxes is not None:
                if contact_flag[i] == 0:
                    self.draw_box(boxes[i], edge_color=color)
                else:
                    self.draw_box_MC50(boxes[i], edge_color=color)
            ...
            if labels is not None:
                ...
                if boxes is not None:
                    x0, y0, x1, y1 = boxes[i]
                    text_pos = (x0, y0)
                    text_pos_obj_obj_contact = (x0, y1 - 38) # if drawing boxes, put text on the box corner.
                    text_pos_obj_hand_contact = (x1 - 32, y1 - 32)
                    horiz_align = "left"
                elif masks is not None:
                ...
                self.draw_text_MC50(
                    obj_obj_contact_pos=text_pos_obj_obj_contact,
                    obj_hand_contact_pos=text_pos_obj_hand_contact,
                    obj_obj_contact_classes=obj_obj_contact_classes[i],
                    obj_obj_contact_scores=obj_obj_contact_scores[i],
                    obj_hand_contact_classes=obj_hand_contact_classes[i],
                    obj_hand_contact_scores=obj_hand_contact_scores[i],
                    text=labels[i],
                    position = text_pos,
                    color=lighter_color,
                ...
                # if not 'hand' in labels[i]:
                #     color_connection = _CONNECTION_OBJ_OBJ
                # else:
                #     color_connection = _CONNECTION_OBJ_HAND
                #draw obj-obj-connection
                if contact_flag[i] == 1:
                    for contacted_obj in Contact_infos[i]:
                        self.draw_contact_MC50(
                            boxes[i, :],
                            boxes[contacted_obj, :],
                            color = _CONNECTION_OBJ_OBJ
                        )

                # draw obj-obj-connection
                if contact_hand_flag[i] == 1:
                    for contacted_obj in Contact_hand_infos[i]:
                        self.draw_contact_MC50(
                            boxes[i, :],
                            boxes[contacted_obj, :],
                            color = _CONNECTION_OBJ_HAND
                        )
        ...
        horiz_align = "left"
        self.draw_step(step_infos=step_infos, font_size=15, color=_STEP_COLOR, horizontal_alignment=horiz_align)

        return self.output

    def find_errors(self, step_infos):
        current_step, current_sub_step, next_sub_step = step_infos
        if len(current_sub_step) <= 1:
            return None
        else:
            if current_sub_step[-1] != next_sub_step[-2]:
                print_ = 'this step should be ' + next_sub_step[-2] + ', but get ' + current_sub_step
                return print_

    def draw_step_and_errors(self,
                             step_infos,
                             errors,
                             font_size,
                             color,
        ...
        s = 1
        context_list = []
        if step_infos == 'Need more test !' or step_infos == ([], [], []):
            context_list.append('Need more test !')
        else:
            if errors is None:
                _error = 'None'
            else:
            ...
            #     _error = 'None'
            # else:
            context_list.append('Current_step : ' + current_step[-1])
            context_list.append('Current_sub_step : ' + current_sub_step[-1])
            context_list.append('Next_step : ' + next_sub_step[-1])
            context_list.append('Errors : ' + _error)
        ...
        if not step_infos[0] == 'Need more test !':
            ...
            if sub_step[-1] == -1:
                ...
                if 'end_frame' in list(sub_step[-1].keys()):
                    _current_sub_step =  sub_step[-1]['sub-step'] + ' (finished)'
                else:
                    _current_sub_step = sub_step[-1]['sub-step']
                if '(finished)' in _current_sub_step or '(finished)' in _current_step:
                    context_list.append('Pred: background')
                else:
                    context_list.append('Pred:')
                    context_list.append('current step : ' + _current_step)
                    context_list.append('current sub-step : ' + _current_sub_step)
    ...
    def draw_instance_predictions_filter(self, box_info, predictions):
        """
        Draw instance-level prediction results on an image.
        ...
        # filter the boxes by iou

        self.overlay_instances_filter(
            masks=masks,
            boxes=boxes,
            labels=labels,
            keypoints=keypoints,
            assigned_colors=colors,
            alpha=alpha, box_info = box_info
        )
        return self.output
    ...
    def overlay_instances(
        ...
        *,
        ...
        _ASSIGN_COLOR = True
        # color_mapping = {}
        # # color_mapping['plate'] = [250/255, 250/255, 250/255]
        # # color_mapping['tomato'] = [255/255, 0/255, 0/255]
        # # color_mapping['apple'] = [178/255,34/255, 34/255]
        # # color_mapping['bread'] = [139/255, 90/255, 43/255]
        # # color_mapping['milk'] = [0/255, 191/255, 255/255]
        # # color_mapping['carrot'] = [255/255, 140/255, 0/255]
        # # color_mapping['cheese'] = [255/255, 215/255, 0/255]
        # # color_mapping['ham'] = [238/255, 180/255, 180/255]
        # # color_mapping['lettuce'] = [0/255, 255/255, 127/255]
        # # color_mapping['mug'] = [100/255, 149/255, 237/255]
        # color_mapping['coffee bag'] = [139/255, 115/255, 85/255]
        # color_mapping['coffee grounds'] = [139/255, 69/255, 19/255]
        # color_mapping['filter cone (empty)'] = [205/255, 201/255, 201/255]
        # color_mapping['filter cone + paper filter (full)'] = [205/255, 192/255, 176/255]
        # color_mapping['grinder'] = [138/255, 43/255, 226/255]
        # color_mapping['kettle'] = [54/255, 54/255, 54/255]
        # color_mapping['measuring cup'] = [209/255, 238/255, 238/255]
        # color_mapping['mug'] = [100/255, 149/255, 237/255]
        # color_mapping['mug + coffee'] = [139/255, 115/255, 85/255]
        # color_mapping['mug + filter cone (empty)'] = [110/255, 123/255, 139/255]
        # color_mapping['mug + filter cone + paper filter (empty)'] = [255/255, 131/255, 250/255]
        # color_mapping['mug + filter cone + paper filter (full)'] = [238/255, 58/255, 140/255]
        # color_mapping['paper filter (full)'] = [205/255, 149/255, 12/255]
        # color_mapping['paper filter (quarter)'] = [255/255, 185/255, 15/255]
        # color_mapping['paper filter (semi)'] = [238/255, 173/255, 14/255]
        # color_mapping['paper towel'] = [255/255, 255/255, 224/255]
        # color_mapping['scale'] = [34/255, 139/255, 34/255]
        # color_mapping['thermometer'] = [0/255, 255/255, 255/255]
        # color_mapping['timer'] = [188/255, 238/255, 104/255]
        # color_mapping['water'] = [176/255, 196/255, 222/255]
        ...
        for i in range(num_instances):
            ...
            if _ASSIGN_COLOR:
                len_score = len(labels[i].split(' ')[-1])
                label_ = labels[i][: (-len_score - 1)]
                color_mapping = self.load_color_mapping()
                color = color_mapping[label_]
    ...
    def overlay_instances_MC50(
        ...
        *,
        ...
        # self.tracker.update(predictions=instances, current_idx=current_idx)
        ...
        for i in range(num_instances):
            if boxes is not None:
                ...
                for value in boxes[i]:
                    box_i.append(value)
                #iou = self.bb_intersection_over_union(self, box_info, box_i)
                boxA = box_info
                boxA = [boxA[2], boxA[0], boxA[3], boxA[1]]
                boxB = box_i
                boxA = [int(x) for x in boxA]
                boxB = [int(x) for x in boxB]
    ...
    def draw_text_MC50(
        ...
        *,
        ...
        x_, y_ = obj_obj_contact_pos

        _x, _y = obj_hand_contact_pos

        percent = text.split(' ')[-1]
        class_list = text[:-(len(percent) +1 )].split('+')
        class_list.append(percent)


        for _text in class_list:
            if _text[0] == ' ':
                _text = _text[1:]
            if _text[-1] == ' ':
                _text = _text[:-1]
        ...
        # add the contact labels
        obj_obj_contact_scores = str(int(obj_obj_contact_scores * 100)) + '%'
        if obj_obj_contact_scores != '100%':
            obj_obj_contact_scores = ' ' + obj_obj_contact_scores
        self.output.ax.text(
            x_,
            y_,
        ...
        # add the contact labels
        obj_hand_contact_scores = str(int(obj_hand_contact_scores * 100)) + '%'
        if obj_hand_contact_scores != '100%':
            obj_hand_contact_scores = ' ' + obj_hand_contact_scores
        self.output.ax.text(
            _x,
            _y,
    ...
    def draw_contact_MC50(
        self,
        boxes_1,
        boxes_2,
        color
    ):
        cirle_1_x = (boxes_1[0] + boxes_1[2])/2
        cirle_1_y = (boxes_1[1] + boxes_1[3]) / 2
        cirle_2_x = (boxes_2[0] + boxes_2[2]) / 2
        cirle_2_y = (boxes_2[1] + boxes_2[3]) / 2


        self.draw_line([cirle_1_x, cirle_2_x], [cirle_1_y, cirle_2_y], color=color, linewidth=6)
        self.draw_circle((cirle_1_x, cirle_1_y), color=color, radius=20)
        self.draw_circle((cirle_2_x, cirle_2_y), color=color, radius=20)
    ...
    def draw_box_MC50(self, box_coord, alpha=0.5, edge_color="g", line_style="-"):
        """
        Args:
            box_coord (tuple): a tuple containing x0, y0, x1, y1 coordinates, where x0 and y0
                are the coordinates of the image's top left corner. x1 and y1 are the
...
class Visualizer_eval:
    ...
    def draw_instance_predictions_smoothing(self, predictions, tracker, current_idx):
        ...
        step_info = self.overlay_instances_MC50_add_smoothing(
            masks=masks,
            boxes=boxes,
            labels=labels,
            keypoints=keypoints,
