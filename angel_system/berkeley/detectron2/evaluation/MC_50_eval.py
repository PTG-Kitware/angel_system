...
class LVISEval:
    ...
    def _prepare(self):
        ...
        for dt in dts:
            ...
            if cat_id not in img_pl[img_id]:
                continue
            self._dts[img_id, cat_id].append(dt)

        #prepare cate-based eval

        # self.freq_groups = self._prepare_freq_group()
        # self.cate_groups = self._prepare_cate_group()
    ...
    def _prepare_freq_group(self):
        ...
        #change the fre
        temp = []
        for _cate in cat_data:
            _cate['frequency'] = _cate['name']
            temp.append(_cate['name'])
        print(temp)
    ...
    def summarize(self):
        ...
        #     self.results["AP_" + str(i)] = self._summarize('ap', freq_group_idx=i)
