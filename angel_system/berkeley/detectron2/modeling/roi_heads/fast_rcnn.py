...
def fast_rcnn_inference_plus_contact(
    boxes: List[torch.Tensor],
    scores: List[torch.Tensor],
    obj_obj_scores: List[torch.Tensor],
    obj_hand_scores: List[torch.Tensor],
    image_shapes: List[Tuple[int, int]],
    score_thresh: float,
    nms_thresh: float,
    topk_per_image: int,
...
def filter_contact_score(contact_scores):
    # filter the contact score
    new_contact_score = []
    pred_contact_state = []
    for ins in contact_scores:
        have_contact_score = ins[1]
        have_no_contact_score = ins[0]
        if have_no_contact_score > have_contact_score:
            new_contact_score.append(ins[0])
            pred_contact_state.append(0)
        else:
...
def fast_rcnn_inference_single_image_plus_contact(
    ...
    obj_obj_scores = obj_obj_scores[:, 1:]
    obj_hand_scores = obj_hand_scores[:, 1:]
    ...
    result.obj_obj_contact_scores = torch.tensor(obj_obj_contact_scores).cuda()
    result.obj_obj_contact_classes = torch.tensor(obj_obj_contact_classes).cuda()
    result.obj_hand_contact_scores = torch.tensor(obj_hand_contact_scores).cuda()
    result.obj_hand_contact_classes = torch.tensor(obj_hand_contact_classes).cuda()
    ...
    # result.pred_boxes = Boxes(boxes)
    # result.scores = scores
    # result.contact_scores = torch.tensor(new_contact_score).cuda()
...
def fast_rcnn_inference_contact(
    boxes: List[torch.Tensor],
    scores: List[torch.Tensor],
    contact_scores: List[torch.Tensor],
    image_shapes: List[Tuple[int, int]],
    score_thresh: float,
...
def _log_contact_state_stats(pred_logits, gt_contact_state, prefix="fast_rcnn"):
    ...
    Args:
        ...
        gt_classes: R labels
    """
    num_instances = gt_contact_state.numel()
    if num_instances == 0:
        return
    pred_contact_state = pred_logits.argmax(dim=1)
    bg_contact_state_ind = pred_logits.shape[1] - 1

    fg_inds = (gt_contact_state >= 0) & (gt_contact_state < bg_contact_state_ind)
    num_fg = fg_inds.nonzero().numel()
...
def fast_rcnn_inference_single_image_contact(
    ...
    scores = scores[:, :-1]
    contact_scores = contact_scores[:, 1:]
    num_bbox_reg_classes = boxes.shape[1] // 4
    # Convert to Boxes to use the `clip` function ...
    boxes = Boxes(boxes.reshape(-1, 4))
    boxes.clip(image_shape)
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def inference_mixed(self, predictions: Tuple[torch.Tensor, torch.Tensor], predictions_contact: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        obj_obj_scores = self.predict_contact_probs(predictions_contact[0], proposals)
        obj_hand_scores = self.predict_contact_probs(predictions_contact[1], proposals)
        image_shapes = [x.image_size for x in proposals]
        return fast_rcnn_inference_plus_contact(
            boxes,
            scores,
            obj_obj_scores,
            obj_hand_scores,
...
class FastRCNNOutputLayers_PLUS_CONTACT(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
    ...
    def __init__(
        ...
        *,
        ...
        nn.init.normal_(self.bbox_pred.weight, std=0.001)
        for l in [self.cls_score, self.bbox_pred, self.contact_pred]:
            nn.init.constant_(l.bias, 0)

        self.box2box_transform = box2box_transform
        self.smooth_l1_beta = smooth_l1_beta
        self.test_score_thresh = test_score_thresh
        self.contact_score_thresh = contact_score_thresh
        self.test_nms_thresh = test_nms_thresh
        self.test_topk_per_image = test_topk_per_image
        self.box_reg_loss_type = box_reg_loss_type
        if isinstance(loss_weight, float):
    ...
    def forward(self, x):
        ...
        scores = self.cls_score(x)
        proposal_deltas = self.bbox_pred(x)
        contact = self.contact_pred(x)
        return scores, proposal_deltas, contact
    ...
    def losses(self, predictions, proposals):
        ...
        Returns:
            Dict[str, Tensor]: dict of losses
        """
        scores, proposal_deltas, contact = predictions
        ...
        losses = {
            "loss_cls": loss_cls,
            "loss_contact_state": loss_contact_state,
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes
            ),
        }
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        """
        boxes = self.predict_boxes(predictions, proposals)
        scores = self.predict_cls_probs(predictions, proposals)
        contact_scores = self.predict_contact_probs(predictions, proposals)
        image_shapes = [x.image_size for x in proposals]
        return fast_rcnn_inference_contact(
            boxes,
            scores,
            contact_scores,
            image_shapes,
            self.test_score_thresh,
            self.contact_score_thresh,
            self.test_nms_thresh,
            self.test_topk_per_image,
        )
    ...
    def predict_boxes(
        ...
        if not len(proposals):
            return []
        _, proposal_deltas, contact_state = predictions
        num_prop_per_image = [len(p) for p in proposals]
        proposal_boxes = cat([p.proposal_boxes.tensor for p in proposals], dim=0)
        predict_boxes = self.box2box_transform.apply_deltas(
            proposal_deltas,
...
class FastRCNNOutputLayers_CONTACT(nn.Module):
    ...
    def forward(self, x):
        ...
        # scores = self.cls_score(x)
        # proposal_deltas = self.bbox_pred(x)
        obj_obj_contact_pred = self.obj_obj_contact_pred(x)
        obj_hand_contact_pred = self.obj_hand_contact_pred(x)
        return obj_obj_contact_pred, obj_hand_contact_pred
