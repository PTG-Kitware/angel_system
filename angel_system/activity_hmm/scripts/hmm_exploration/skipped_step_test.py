...
from angel_system.activity_hmm.eval import (
...
config_fname = (
    base_path + "/../config/tasks/task_steps_config-recipe_coffee_trimmed_v3.yaml"
)
...
for i in range(N):
    ...
    live_model.add_activity_classification(
        range(live_model.num_activities), X[i], time_windows[i, 0], time_windows[i, 1]
    )
