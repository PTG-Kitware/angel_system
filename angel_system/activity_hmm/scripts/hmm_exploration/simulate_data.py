...
dt = 0.25
ret = load_and_discretize_data(activity_gt, extracted_activity_detections, dt, 0.5)
...
fp = [[X[i, j] for j in range(X.shape[1]) if Z[i] != j] for i in range(len(X))]
fp = np.array(fp).ravel()
print("True examples mean:", np.mean(tp), "std:", np.std(tp))
print("False examples mean:", np.mean(fp), "std:", np.sqrt(np.mean(np.mean(fp**2))))
# ----------------------------------------------------------------------------
...
for _ in range(25):
    ...
    live_model.add_activity_classification(
        live_model.model.class_str, conf_vec, start_time, end_time
    )
    curr_step += 1
    start_time += 1
    end_time += 1
...
plt.tick_params(axis="y", which="minor", grid_linestyle="--", grid_color="lightgrey")
plt.grid(axis="y", which="both")
plt.gca().xaxis.set_minor_locator(AutoMinorLocator(2))
plt.tick_params(axis="x", which="major", grid_color="lightgrey")
plt.tick_params(axis="x", which="minor", grid_linestyle="--", grid_color="lightgrey")
...
model3 = ActivityHMM(
    ...
    class_str,
    med_class_duration=med_class_duration,
    num_steps_can_jump_fwd=1000,
    num_steps_can_jump_bck=1000,
    class_mean_conf=class_mean_conf,
    class_std_conf=class_std_conf,
)
...
for i in range(len(ind) - 1):
    ...
    results.append([scipy.stats.mode(Z0[ind[i] : (ind[i + 1] + 1)]).mode[0], Z1])
    print(results[-1])
...
end_time = 1
i = 1
conf_vec = np.ones(len(live_model.model.class_str)) * (-10000)
conf_vec[i] = 1
live_model.add_activity_classification(
    live_model.model.class_str, conf_vec, start_time, end_time
)
...
for snr in [8]:
    ...
    print(snr)
    np.savetxt(
        "%s/pr_snr=%0.5f.txt" % (save_dir, snr),
