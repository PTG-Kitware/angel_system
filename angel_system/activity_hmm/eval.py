...
def viz_step_finished_conf(
    live_model, X, true_step, time_windows, valid, save_fname=None
):
    ...
    for i in range(N):
        ...
        live_model.add_activity_classification(
            range(live_model.num_activities),
            X[i],
            time_windows[i, 0],
            time_windows[i, 1],
        )
        ...
        truth_step_finished = [
            s in set(true_step[:i]) for s in range(1, live_model.model.num_steps)
        ]
    ...
    plt.imshow(
        running_step_finished_conf,
        extent=[times[0], times[-1], len(running_step_finished_conf) + 0.5, 0.5],
        aspect="auto",
    ...
    plt.imshow(
        running_unfiltered_step_conf,
        extent=[times[0], times[-1], len(running_unfiltered_step_conf) - 0.5, -0.5],
        aspect="auto",
        interpolation="nearest",
        cmap="cividis",
    )
...
def save_matrix_image(
    ...
    if col_labels:
        ...
        for i in range(len(xs) - 1):
            dx = int((xs[i + 1] - xs[i]))
            cv2.line(
                out, (xs[i], 0), (xs[i], out.shape[0]), (255, 255, 255), thickness=2
            )
    ...
    for i in range(len(ys) - 1):
        dy = int((ys[i + 1] - ys[i]))
        cv2.line(out, (0, ys[i]), (out.shape[1], ys[i]), (255, 255, 255), thickness=2)
