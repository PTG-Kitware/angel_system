...
def main(args):
    ...
    for i in range(len(os.listdir(args.label_dir))):
        ...
        if not args.from_video:
            ...
            for label in range(
                0, (len(pd.unique(df["# 1: Detection or Track-id"])) - 1)
            ):
        ...
        for line in lines:
            ...
            num_labels = len(
                [
                    ...
                    for file in os.listdir(
                        "{}/{}/".format(args.data_out_path, args.phase)
                    )
                    if int(label) == int(file.split("_")[0])
                ]
            )
            ...
            if args.from_video:
                ...
                for frame in sorted(os.listdir(image_folder)):
                    if (
                        int(frame[6:11]) >= frame_start
                        and int(frame[6:11]) <= frame_end
                    ):
        ...
        if args.phase == "train":
            ...
            train_df.to_feather(
                args.label_out_path + "labels_train_{}.feather".format(i)
            )
            train_meta_df.to_feather(
                args.label_out_path + "meta_train_{}.feather".format(i)
            )
        else:
            ...
            test_meta_df.to_feather(
                args.label_out_path + "meta_test_{}.feather".format(i)
            )
    ...
    for file in files:
        ...
        for i in range(len(os.listdir(args.label_dir))):
            temp_df = pd.read_feather(
                args.label_out_path + "{}_{}.feather".format(file, i)
            )
            final_df = pd.concat([final_df, temp_df]).reset_index(
