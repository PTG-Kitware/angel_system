...
# Example:
# python calc_det.py --out test.csv --cuda
...
def get_detections_from_im(fasterRCNN, classes, im_file, image_id, args, conf_thresh=0.1):
    """obtain the image_info for each image,
    im_file: the path of the image

    return: dict of {'image_id', 'image_h', 'image_w', 'num_boxes', 'boxes', 'objects', 'features'}
    ...
    for i in range(len(keep_boxes)):
        ...
        box_dets[i] = bbox.cpu().numpy()

    sample_info = dict(classes=classes[1:], boxes=box_dets, objects=objects.cpu().numpy(), feats=pooled_feat[keep_boxes].cpu().detach().numpy())
...
def generate_tsv(frame_path, image_ids, args):
    classes, fasterRCNN = load_model(args)
    for k in tqdm.tqdm(range(len(image_ids))):
        image_file = image_ids[k]
        image_path = os.path.join(frame_path, image_file)
        fsplit = image_path.strip().split('/')
        det_path = '/'.join(fsplit[:-2] + ['det'])
