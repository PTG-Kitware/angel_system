...
TSN Configurations:
    ...
    def _construct_flow_model(self, base_model):
        ...
        first_conv_idx = list(
            filter(lambda x: isinstance(modules[x], nn.Conv2d), list(range(len(modules))))
        )[0]
        ...
        new_kernels = (
            params[0].detach().mean(dim=1, keepdim=True).expand(new_kernel_size).contiguous()
        )
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if not keep_rgb:
            ...
            new_kernel_size = (
                kernel_size[:1] + (3 + 3 * self.new_length["RGBDiff"],) + kernel_size[2:]
            )
