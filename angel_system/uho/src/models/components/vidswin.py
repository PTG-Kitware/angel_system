...
class VideoSwinTransformerBackbone(nn.Module):
    ...
    def forward(self, samples: torch.Tensor):
        ...
        samples = rearrange(samples, 'b t c h w -> b c t h w')
        ...
        out[-1] = rearrange(out[-1], 'b c t h w -> b t c h w')
        new_out = out[-1]
        a = new_out.shape[0]
        new_out = new_out.reshape(-1,new_out.shape[2],new_out.shape[3],new_out.shape[4])
        new_out = nn.MaxPool2d(kernel_size=(new_out.shape[-2], new_out.shape[-1]))(new_out).squeeze().reshape(a,-1,new_out.shape[1])
        return new_out
...
def video_swin_t(pretrain_path):
    cfgs = configs['video_swin_t_p4w7']
    model = VideoSwinTransformerBackbone(True, pretrain_path,
                                         True, **cfgs)
    return model
