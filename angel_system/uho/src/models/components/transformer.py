...
class TemTRANSModule(nn.Module):
    def __init__(self, act_classes, hidden, dropout=0.0, depth=4, num_head=8):
        super().__init__()
        self._det_topk = 10
        ...
        self.transformer = TemporalTransformer(
            width=hidden * 2, layers=depth, heads=num_head
        )
        self.classifier_action = nn.Sequential(
            nn.Dropout(dropout), nn.Linear(hidden * 2, act_classes)
        )
        self.classifier_detection = nn.Sequential(
            nn.Dropout(dropout), nn.Linear(hidden * 2, det_classes)
        )
        # Loss functions
        self.loss_detection = nn.CrossEntropyLoss(label_smoothing=0.1)
        class_weight = torch.ones(act_classes)
        class_weight[0] = 2.0 / float(act_classes)
    ...
    def forward(self, inputs, if_train):
        ...
        lh, rh = (
            inputs[0]["labels"]["l_hand"],
            inputs[0]["labels"]["r_hand"],
        )  # hand poses
        ...
        feat_b[:, :, [0, 2]] = feat_b[:, :, [0, 2]] / 1280.0
        feat_b[:, :, [1, 3]] = feat_b[:, :, [1, 3]] / 720.0
        ...
        detection_loss = self.loss_detection(
            detection_out.permute(0, 2, 1), det_labels.long()
        )
    ...
    def predict(self, inputs):
        ...
        valid_fr = [
            k
            for k in range(feat_x.shape[1])
            if torch.sum(tmp[:, k * topK : (k + 1) * topK]) != 0
        ]
