...
class RULSTM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.fc1(x)
        if x1.ndim == 2: x1 = x1.unsqueeze(1)
        x1 = self.fc2(x1)
        
        #print(x1.ndim, x.shape, x1.shape)
        x1 = x1.permute(1,0,2).reshape(x.shape[1],x.shape[0],x.shape[2]//2).permute(1,0,2).contiguous()

        h = torch.cat([x1, h], axis=-1)
