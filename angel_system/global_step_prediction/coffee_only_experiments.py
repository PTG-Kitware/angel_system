...
if __name__ == "__main__":
    ...
    video_dset = coco_test.subset(gids=image_ids, copy=True)
    activity_confs = video_dset.images().lookup("activity_conf")
    step_gts = get_gt_steps_from_gt_activities(video_dset)[0]
    # part 2
    vid_id = 2
    image_ids = coco_test.index.vidid_to_gids[vid_id]
    video_dset = coco_test.subset(gids=image_ids, copy=True)
    activity_confs.extend(video_dset.images().lookup("activity_conf"))
    step_gts.extend(get_gt_steps_from_gt_activities(video_dset)[0])
