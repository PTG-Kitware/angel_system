...
class H2OVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self._parse_annotationfile(self.use_feats)
    ...
    def _load_feats(self, directory: str, idx: int) -> Dict:
        return torch.load(os.path.join(directory, self.imagefile_template.format(idx) + ".pk"))

    def _parse_annotationfile(self, use_feats=False):
        ann_file = open(self.annotationfile_path)
        self.video_list = [
            VideoRecord(x.strip().split(), self.root_path, use_feats)
